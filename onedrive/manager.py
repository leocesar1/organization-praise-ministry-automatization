...
class OneDrive(metaclass=Metaclass):
    ...
    def connect(self, client_id=None):
        ...
        client_id = (
            ...
            else get_Credentials("onedrive")["onedrive"]["client_id"]
        )
        try:
            print(
    ...
    def getFileList(self, pathId="A9BCF86E0D3403C2!367937"):
        ...
        response = loads(get(url, headers=self.HEADERS).text)
        result = response["value"]
        while "@odata.nextLink" in response.keys():
            url = response["@odata.nextLink"]
            response = loads(get(url, headers=self.HEADERS).text)
            [result.append(item) for item in response["value"]]

        with open("message/list.json", "w", encoding="utf-8") as file:
            file.write(dumps(result))
        return result

    def downloadFile(self, downloadUrl):
        return get(downloadUrl, headers=self.HEADERS, allow_redirects=True).content
